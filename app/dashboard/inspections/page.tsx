  ...
  return (
    <section aria-labelledby="inspections-heading">
      <header>
        ...
        <h1 id="inspections-heading">Inspections</h1>
        <p>Review inspection activity across the property portfolio.</p>
      </header>

      {inspections.length === 0 ? (
        <p>No inspections are currently recorded.</p>
      ) : (
        <ul>
          {inspections.map(({ property, inspection }) => (
            <li key={inspection.id}>
              ...
              <span> · {String(inspection.status)} · {String(inspection.scheduled_at ?? inspection.created_at)}</span>
            </li>
          ))}
        </ul>
      )}
