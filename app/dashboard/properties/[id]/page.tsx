...
export default async function PropertyDetailPage({
  ...
  const [inspectionsResult, issuesResult] = await Promise.all([
    listPropertyInspections(supabase, id),
    listPropertyIssues(supabase, id),
  ]);

  if (inspectionsResult.error || issuesResult.error) {
  ...
  return (
    <section aria-labelledby="property-heading">
      ...
      <header>
        <p>Property</p>
        <h1 id="property-heading">{property.name}</h1>
        <p>{property.address ?? "Address not recorded"}</p>
      </header>

      <section aria-label="Property summary">
        <article><h2>{inspections.length}</h2><p>Inspections</p></article>
        <article><h2>{openIssues.length}</h2><p>Open issues</p></article>
        <article><h2>{issues.length}</h2><p>Total issues</p></article>
      </section>

      <section aria-labelledby="inspection-heading">
        <h2 id="inspection-heading">Inspection history</h2>
          ...
          <ul>
            {inspections.map((inspection) => (
              <li key={inspection.id}>
                <strong>{String(inspection.status)}</strong>
                <span> · {String(inspection.scheduled_at ?? inspection.created_at)}</span>
              </li>
            ))}
          </ul>
        )}
      ...
      <section aria-labelledby="issues-heading">
        ...
        {issues.length === 0 ? (
          ...
          <ul>
            {issues.map((issue) => (
              <li key={issue.id}>
                <strong>{String(issue.status)}</strong>
