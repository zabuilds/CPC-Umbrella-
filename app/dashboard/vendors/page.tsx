  ...
  return (
    <section aria-labelledby="vendors-heading">
      <header>
        ...
        <h1 id="vendors-heading">Vendors</h1>
        <p>Maintain the trusted service-provider network supporting the property portfolio.</p>
      </header>

      {vendors.length === 0 ? (
        <p>No vendors are currently recorded.</p>
      ) : (
        <ul>
          {vendors.map((vendor) => (
            <li key={vendor.id}>
              <strong>{vendor.name}</strong>
              <span> · {vendor.category ?? "Category not recorded"}</span>
            </li>
          ))}
        </ul>
      )}
